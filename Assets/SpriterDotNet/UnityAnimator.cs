...
namespace SpriterDotNetUnity
{
    public class UnityAnimator : Animator<Sprite, AudioClip>
    {
        ...
        protected override void ApplySpriteTransform(Sprite sprite, SpriterObject info)
        {
            ...
            /*
            if (float.IsNaN(info.X)) {info.X = pivotTransform.localPosition.x;}
            if (float.IsNaN(info.Y)) {info.Y = pivotTransform.localPosition.y;}
            
            if (float.IsNaN(info.PivotX)) {info.PivotX = 0;}
            if (float.IsNaN(info.PivotY)) {info.PivotY = 0;}
            
            if (float.IsNaN(info.ScaleX)) {info.ScaleX = pivotTransform.localScale.x;}
            if (float.IsNaN(info.ScaleY)) {info.ScaleY = pivotTransform.localScale.y;}
            */
